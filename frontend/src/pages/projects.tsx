...
function SpinningCard({
  ...
  return (
    <>
      <div className="flex flex-col justify-center items-center spinningCard text-black" id={CardId}>
        <img src={imgUrl} className="h-20 w-20 bg-cover bg-no-repeat"/>
        <h2 className="text-xl">{heading}</h2>
        <p className="text-sm">{discription}</p>
        <a target="_blank" className="text-base py-2 px-5 bg-primary rounded-md text-white hover:text-white" href={buttonLink}>Learn More</a>
      </div>
    </>
  );
}
